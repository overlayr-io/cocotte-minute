...
describe('PeopleService', () => {
  describe('listMine', () => {
    it('assemble chaque personne avec ses tags associés (sans N+1)', async () => {
      ...
      // people + person_tags = 2 requêtes seulement, hydratation groupée.
  ...
  describe('addTag', () => {
    it('lève NotFound si la personne appartient à un autre compte', async () => {
      ...
      const service = new PeopleService(db, fakeTagsService([]));
    ...
    it('associe le tag et retourne la personne hydratée', async () => {
      // 1) findOwnedOrFail select, 2) insert pivot, 3) hydrate select links
      const { db, calls } = makeDb([
        [personRow()],
        undefined,
        [{ tagId: 'tag-a' }],
