...
class SearchRepositoryException implements Exception {
  const SearchRepositoryException(this.message, {this.premiumLimit});

  final String message;

  /// Limite freemium (403 `PREMIUM_LIMIT_SEARCH_CRITERIA`) : l'UI ouvre la
  /// feuille d'upsell au lieu d'afficher le message brut.
  ...
  SearchRepositoryException _mapError(DioException e, String fallback) {
    ...
    if (status == 400 || status == 403 || status == 404 || status == 409) {
      ...
      if (data is Map && data['message'] is String) {
        return SearchRepositoryException(
          data['message'] as String,
          premiumLimit: premiumLimit,
        );
      }
      if (premiumLimit != null) {
        return SearchRepositoryException(fallback, premiumLimit: premiumLimit);
      }
    }
